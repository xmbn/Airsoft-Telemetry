  ...
  group('TelemetryService Unit Tests', () {
    ...
    group('Initialization', () {
      ...
      test('should load preferences correctly during initialization', () async {
        ...
        verify(() => mockPreferencesService.getIntervalInSeconds('5 seconds'))
            .called(1);
      });
    ...
    group('Session Management', () {
      ...
      test('should pause session successfully', () async {
        ...
        expect(telemetryService.currentSessionId, isNotNull);
        verify(() => mockDatabaseService.insertEvent(any()))
            .called(2); // START + PAUSE
      });
    ...
    group('Event Recording', () {
      ...
      test('should record manual event during active session', () async {
        ...
        verify(() => mockDatabaseService.insertEvent(any()))
            .called(2); // START + HIT
        verify(() => mockLocationService.getCurrentPosition())
            .called(3); // init + startSession + recordEvent
      });

      test('should auto-start session when recording event while stopped',
          () async {
        // Arrange
        expect(telemetryService.sessionState, equals(SessionState.stopped));
      ...
      test('should create event with correct data structure', () async {
        ...
        // Assert
        final capturedCalls =
            verify(() => mockDatabaseService.insertEvent(captureAny()))
                .captured;
        ...
        expect(
            killEvent.gameSessionId, equals(telemetryService.currentSessionId));
        ...
        expect(killEvent.timestamp,
            closeTo(DateTime.now().millisecondsSinceEpoch, 1000));
      });

      test('should verify exact parameters passed to database service',
          () async {
        // Arrange
        await telemetryService.startSession();
        ...
        expect(eliminationEvent.gameSessionId,
            equals(telemetryService.currentSessionId));
      ...
      test('should handle NaN values in position data', () async {
        ...
        expect(testEvent.accuracy,
            equals(nanPosition.accuracy)); // Non-NaN should be preserved
      });

      test('should handle location service errors during event recording',
          () async {
    ...
    group('Stream Notifications', () {
      ...
      test('should emit position updates', () async {
        ...
        final positionFuture = expectLater(
          ...
          emitsInOrder([
            predicate<Position?>(
                (pos) => pos?.latitude == testPosition.latitude),
            predicate<Position?>(
                (pos) => pos?.latitude == testPosition.latitude),
          ]),
        );
    ...
    group('Settings Updates', () {
      ...
      test('should update player name', () async {
        ...
        verify(() => mockPreferencesService.setPlayerName('new_player'))
            .called(1);
      });
      ...
      test('should update tracking interval', () async {
        ...
        // Assert
        verify(() => mockPreferencesService.setUpdateInterval('3 seconds'))
            .called(1);
        verify(() => mockPreferencesService.getIntervalInSeconds('3 seconds'))
            .called(1);
      });

      test(
          'should restart location tracking when updating interval during active session',
          () async {
        // Arrange
        await telemetryService.startSession();
        expect(telemetryService.sessionState, equals(SessionState.running));
        ...
        // Assert
        expect(telemetryService.sessionState, equals(SessionState.running));
        verify(() => mockPreferencesService.setUpdateInterval('10 seconds'))
            .called(1);
      });
    });
    ...
    group('Error Handling', () {
      ...
      test('should handle location service errors during session start',
          () async {
    ...
    group('Disposal', () {
      test('should dispose resources properly', () async {
        ...
        verify(() => mockLocationService.dispose())
            .called(2); // called in test and tearDown
      });
    ...
    group('Complete Session Flow', () {
      ...
      test('should handle complete session lifecycle with events', () async {
        ...
        expect(
            events,
            hasLength(
                7)); // START, HIT, KILL, PAUSE, RESUME, OBJECTIVE_COMPLETE, STOP
    ...
    group('Null Safety and Edge Cases', () {
      ...
      test('should handle null position gracefully during session start',
          () async {
        // Arrange
        when(() => mockLocationService.getCurrentPosition())
      ...
      test('should handle null position gracefully during event recording',
          () async {
        // Arrange
        when(() => mockLocationService.getCurrentPosition())
            .thenAnswer((_) async => testPosition);
