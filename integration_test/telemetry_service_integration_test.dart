...
void main() {
  ...
  group('TelemetryService Integration Tests', () {
    ...
    test('complete session lifecycle creates correct events', () async {
      ...
      expect(sessionEvents.length,
          greaterThanOrEqualTo(5)); // START, HIT, PAUSE, RESUME, KILL, STOP
    ...
    test('event caching updates correctly during session', () async {
      ...
      expect(
          cachedEvents.any((e) => e.eventType == 'OBJECTIVE_CAPTURED'), isTrue);
    ...
    test('stream notifications work during session lifecycle', () async {
      ...
      final stateSubscription =
          telemetryService.sessionStateStream.listen((state) {
    ...
    test('paused session prevents manual event recording', () async {
      ...
      expect(
          finalEvents.any((e) => e.eventType == 'SHOULD_NOT_RECORD'), isFalse);
    });
    ...
    test('location tracking creates periodic LOCATION events', () async {
      ...
      final locationEvents =
          events.where((e) => e.eventType == 'LOCATION').length;
    ...
    test('player name and interval updates work correctly', () async {
      ...
      expect(
          telemetryService.currentPlayerId, equals('integration_test_player'));
      ...
      final testEvent =
          events.firstWhere((e) => e.eventType == 'SETTINGS_TEST');
